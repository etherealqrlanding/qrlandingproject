...
async function prepareCheckoutHold(
  ...
  const { rows: optionRows } = await pool.query<{
    ...
    net_transfer_price_usd_palermo: string | null;
...
checkoutRouter.post('/cash', checkoutLimiter, async (req, res, next) => {
  try {
    ...
    if (netCurrencyCash === 'USD') {
      ...
      const netTransferCashBase = option.net_transfer_price_usd != null ? Number.parseFloat(option.net_transfer_price_usd) : null;
      const netTransferCashPalermo = option.net_transfer_price_usd_palermo != null ? Number.parseFloat(option.net_transfer_price_usd_palermo) : null;
      const netTransferCash = input.transfer_zone === 'palermo' && netTransferCashPalermo != null ? netTransferCashPalermo : netTransferCashBase;
      ...
      const netTransferArsCashBase = option.net_transfer_price_ars != null ? Number.parseFloat(option.net_transfer_price_ars) : null;
      const netTransferArsCashPalermo = option.net_transfer_price_ars_palermo != null ? Number.parseFloat(option.net_transfer_price_ars_palermo) : null;
      const netTransferArsCash = input.transfer_zone === 'palermo' && netTransferArsCashPalermo != null ? netTransferArsCashPalermo : netTransferArsCashBase;
