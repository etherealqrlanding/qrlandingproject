...
// Página pública (sin login) donde el administrador de una cuenta de vendedor que se
// olvidó su PIN elige uno nuevo, a partir del link de un solo uso que le llega por
// email — ver sellerApi.adminPin.forgotPin/resetPin y api/src/routes/seller/index.ts.
// Análoga a ResetMemberPinPage, pero para el PIN de administrador de la cuenta.
export default function ResetAdminPinPage() {
  ...
  useEffect(() => {
    ...
    sellerApi.adminPin.resetPinPreview(token)
      .then((data) => { setSellerName(data.seller_name); setPhase('form'); })
      .catch((err) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setPhase('saving');
    try {
      await sellerApi.adminPin.resetPin(token, pin);
      setPhase('success');
    } catch (err) {
      ...
      <div className="w-full max-w-sm">
        ...
        {(phase === 'form' || phase === 'saving') && (
          <form onSubmit={handleSubmit} className="rounded-2xl border border-gold/15 bg-ink-soft/60 p-8">
            <p className="text-lg font-display text-cream mb-1">PIN de administrador — {sellerName}</p>
            <p className="text-sm text-cream/60 mb-6">Elegí el PIN nuevo (4 a 6 dígitos).</p>
        ...
        {phase === 'success' && (
          <div className="rounded-2xl border border-gold/15 bg-ink-soft/60 p-8 text-center">
            ...
            <p className="text-sm text-cream/60">Ya se puede usar el PIN nuevo la próxima vez que se necesite.</p>
          </div>
        )}
      </div>
    </div>
