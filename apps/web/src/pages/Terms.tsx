...
import type { TermsContent } from '../types/api';
  ...
  return (
    <section className="container-narrow py-6 sm:py-16">
      ...
      {content && paragraphs.length > 0 && (
        <Reveal className="mt-8 max-w-2xl space-y-5 text-base leading-relaxed text-cream/80">
          {paragraphs.map((p, i) => (
      ...
      {content && paragraphs.length === 0 && !error && (
        <p className="mt-8 text-cream/50">{t('terms.empty')}</p>
      )}
    </section>
  );
}
