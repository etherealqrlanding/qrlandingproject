  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <section className="rounded-lg border border-gold/15 bg-ink-soft/50 p-6 max-w-2xl">
        ...
        <form onSubmit={handleSave} className="mt-5 grid sm:grid-cols-[1fr_auto] gap-3">
      ...
      {rate && Number(rate) > 0 && (
        <section className="mt-6 rounded-lg border border-gold/10 bg-ink/30 p-5 max-w-2xl">
          <p className="text-xs uppercase tracking-widest text-gold-soft">Ejemplo</p>
          <p className="mt-2 text-sm text-cream/70">
            Una venta de <span className="text-cream">USD 140</span> se cobraría como{' '}
            <span className="text-gold">ARS {(140 * Number(rate)).toLocaleString('es-AR')}</span> en Mercado Pago.
          </p>
        </section>
      )}
    </div>
